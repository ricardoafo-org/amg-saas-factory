...
> `pixel-agents/` is an unrelated library — ignore it entirely.
...
main  ← protected, always deployable
  ...
  └── fix/BUG-XXX           ← bug fixes (use bug ID from docs/bugs/)
  └── ecosystem/xxx         ← tooling, agents, CI
```

**Merge checklist** (must be done in order before merging to main):

1. `npm run type-check` → zero exit
2. `npm test` → all pass
3. `compliance-reviewer` → zero violations
4. `qa-engineer` QA run → verdict PASS or PARTIAL (no blocking bugs)
5. Human manual validation of the golden path
6. PR opened → squash merge to main
